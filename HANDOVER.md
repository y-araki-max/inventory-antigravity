# プロジェクト最終引き継ぎ書 (Strict v13.4)

**重要**: 本ドキュメントは 2026年2月18日時点の最終決定版（v13.4）です。
`TypeError` を完全に解消し、計算ロジックを「完全インライン化」しました。

## 1. 唯一の正解ルール (Single Source of Truth)

### 計算ロジックの自己完結化 (Strict v13.4)
...
### BOSS ID (黒三角)
- **インライン実装**: CSSによる黒い三角形の描画と、ツールチップの表示ロジックも全てインラインで記述されています。
- **デザイン**: ユーザー要望通り「右上に黒い三角」「ホバーで内訳表示」となっています。
...
----
This document serves as the absolute reference for the project (v13.4).
